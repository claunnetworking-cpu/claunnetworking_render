...
def login():
    ...
    try:
        ...
        cursor.execute('SELECT id, password_hash, user_type, name FROM users WHERE email = %s',
                      (data['email'],))
    ...
    try:
        ...
        for job in jobs:
            job_dict = {
                ...
                'is_active': job[12], 'is_featured': job[13], 
                'created_at': str(job[14]), 'expires_at': str(job[15]) if job[15] else None,
                'company_name': job[16]
            }
...
def apply_to_job():
    ...
    try:
        ...
        cursor.execute('SELECT id FROM applications WHERE job_id = %s AND candidate_id = %s',
...
def get_courses():
    ...
    try:
        ...
        if level:
            ...
            params.append(level)
        if modality:
            query += ' AND c.modality = %s'
            params.append(modality)
        if is_free:
            query += ' AND c.is_free = %s'
        ...
        for course in courses:
            course_dict = {
                ...
                'is_free': course[9], 'is_featured': course[10], 'is_active': course[11],
