class AdminFunctions{constructor(){this.apiBaseUrl="/api",this.currentFilters={},this.selectedItems=new Set,this.init()}init(){this.setupEventListeners(),this.loadInitialData()}setupEventListeners(){var e=document.getElementById("select-all");e&&e.addEventListener("change",e=>{this.toggleSelectAll(e.target.checked)}),document.addEventListener("change",e=>{e.target.classList.contains("row-checkbox")&&this.handleRowSelection(e.target)}),document.addEventListener("change",e=>{e.target.id.includes("-filter")&&this.applyFilters()})}loadInitialData(){this.loadPendingData(),this.loadOpportunitiesData(),this.loadCoursesData(),this.loadCompaniesData(),this.loadUsersData(),this.loadAuditData(),this.loadNotificationsData()}loadPendingData(){this.populateTable("pending-table-body",[{id:"pend_001",type:"Vaga",title:"Desenvolvedor Frontend React",company:"TechCorp Brasil",area:"Tecnologia",date:"2025-10-03",status:"Pendente",details:{description:"Vaga para desenvolvedor React com experiência em TypeScript",salary:"R$ 8.000 - R$ 12.000",location:"São Paulo, SP",requirements:["React","TypeScript","Node.js"]}},{id:"pend_002",type:"Curso",title:"Marketing Digital Avançado",company:"InnovateEdu",area:"Marketing",date:"2025-10-02",status:"Pendente",details:{description:"Curso completo de marketing digital com certificação",duration:"40 horas",price:"R$ 499,00",modules:["SEO","Google Ads","Social Media","Analytics"]}},{id:"pend_003",type:"Premium",title:"Plano Empresarial - StartupXYZ",company:"StartupXYZ",area:"Tecnologia",date:"2025-10-01",status:"Pendente",details:{plan:"Empresarial",value:"R$ 999,00/mês",features:["Vagas ilimitadas","Destaque premium","Analytics avançado"]}}],this.renderPendingRow.bind(this))}renderPendingRow(e){return`
            <tr data-id="${e.id}">
                ...
                <td>
                    <button class="btn btn-sm btn-secondary" onclick="adminFunctions.viewDetails('${e.id}', 'pending')">👁️ Ver</button>
                    <button class="btn btn-sm btn-success" onclick="adminFunctions.approveItem('${e.id}')">✅ Aprovar</button>
                    <button class="btn btn-sm btn-danger" onclick="adminFunctions.rejectItem('${e.id}')">❌ Reprovar</button>
                </td>
            </tr>
        `}loadOpportunitiesData(){this.populateTable("opportunities-table-body",[{id:"opp_001",type:"Vaga",title:"Engenheiro de Software Senior",company:"DevSolutions",status:"Ativa",likes:120,views:1500,date:"2025-09-28",applications:45},{id:"opp_002",type:"Vaga",title:"Analista de Marketing Digital",company:"MarketMinds",status:"Fechada",likes:80,views:950,date:"2025-09-15",applications:32},{id:"opp_003",type:"Curso",title:"Bootcamp Full Stack",company:"CodeAcademy",status:"Ativa",likes:200,views:2800,date:"2025-09-20",enrollments:150}],this.renderOpportunityRow.bind(this))}renderOpportunityRow(e){var t="Ativa"===e.status?"status-approved":"status-rejected";"Vaga"===e.type?e.applications:e.enrollments;return`
            <tr data-id="${e.id}">
                ...
                <td>${this.formatDate(e.date)}</td>
                <td>
                    <button class="btn btn-sm btn-secondary" onclick="adminFunctions.viewDetails('${e.id}', 'opportunity')">👁️ Ver</button>
                    <button class="btn btn-sm btn-primary" onclick="adminFunctions.editItem('${e.id}', 'opportunity')">✏️ Editar</button>
                    <button class="btn btn-sm btn-danger" onclick="adminFunctions.deleteItem('${e.id}', 'opportunity')">🗑️ Excluir</button>
                </td>
            </tr>
        `}loadCoursesData(){this.populateTable("courses-table-body",[{id:"course_001",title:"Introdução ao React",institution:"CodeAcademy",students:1500,status:"Ativo",price:"Grátis",rating:4.8,duration:"20 horas"},{id:"course_002",title:"Especialização em UX/UI",institution:"DesignSchool",students:800,status:"Ativo",price:"R$ 499",rating:4.9,duration:"60 horas"},{id:"course_003",title:"Python para Data Science",institution:"DataInstitute",students:1200,status:"Pausado",price:"R$ 299",rating:4.7,duration:"40 horas"}],this.renderCourseRow.bind(this))}renderCourseRow(e){var t="Ativo"===e.status?"status-approved":"status-warning",a="Grátis"===e.price?'<span class="price-free">Grátis</span>':`<span class="price-paid">${e.price}</span>`;return`
            <tr data-id="${e.id}">
                <td><strong>${e.title}</strong><br><small>⭐ ${e.rating} • ${e.duration}</small></td>
                <td>${e.institution}</td>
                <td>👥 ${e.students.toLocaleString()}</td>
                <td><span class="status-badge ${t}">${e.status}</span></td>
                <td>${a}</td>
                <td>
                    <button class="btn btn-sm btn-secondary" onclick="adminFunctions.viewDetails('${e.id}', 'course')">👁️ Ver</button>
                    <button class="btn btn-sm btn-primary" onclick="adminFunctions.editItem('${e.id}', 'course')">✏️ Editar</button>
                    <button class="btn btn-sm btn-warning" onclick="adminFunctions.toggleCourseStatus('${e.id}')">⏸️ Pausar</button>
                    <button class="btn btn-sm btn-danger" onclick="adminFunctions.deleteItem('${e.id}', 'course')">🗑️ Excluir</button>
                </td>
            </tr>
        `}loadCompaniesData(){this.populateTable("companies-table-body",[{id:"comp_001",name:"TechCorp Brasil",cnpj:"12.345.678/0001-99",plan:"Profissional",balance:"R$ 599,00",jobs:5,likes:1200,views:15e3,status:"Ativo"},{id:"comp_002",name:"Innovate Solutions",cnpj:"98.765.432/0001-11",plan:"Básico",balance:"R$ 299,00",jobs:2,likes:500,views:7e3,status:"Ativo"},{id:"comp_003",name:"StartupXYZ",cnpj:"11.222.333/0001-44",plan:"Empresarial",balance:"R$ 0,00",jobs:0,likes:50,views:800,status:"Suspenso"}],this.renderCompanyRow.bind(this))}renderCompanyRow(e){e.status;var t="plan-"+e.plan.toLowerCase();return`
            <tr data-id="${e.id}">
                ...
                <td>💼 ${e.jobs}</td>
                <td>❤️ ${e.likes}</td>
                <td>👁️ ${e.views.toLocaleString()}</td>
                <td>
                    <button class="btn btn-sm btn-secondary" onclick="adminFunctions.viewDetails('${e.id}', 'company')">👁️ Ver</button>
                    <button class="btn btn-sm btn-warning" onclick="adminFunctions.suspendCompany('${e.id}')">⏸️ Suspender</button>
                    <button class="btn btn-sm btn-primary" onclick="adminFunctions.editCompanyPlan('${e.id}')">💰 Plano</button>
                </td>
            </tr>
        `}loadUsersData(){this.populateTable("users-table-body",[{id:"user_001",name:"João da Silva",email:"joao.silva@example.com",type:"Candidato",registered:"2025-09-01",lastAccess:"2025-10-03",status:"Ativo"},{id:"user_002",name:"Maria Oliveira",email:"maria.oliveira@example.com",type:"Recrutador",registered:"2025-08-15",lastAccess:"2025-10-02",status:"Ativo"},{id:"user_003",name:"Carlos Santos",email:"carlos.santos@example.com",type:"Candidato",registered:"2025-07-20",lastAccess:"2025-09-15",status:"Inativo"}],this.renderUserRow.bind(this))}renderUserRow(e){var t="Ativo"===e.status?"status-approved":"status-warning",a="type-"+e.type.toLowerCase();return`
            <tr data-id="${e.id}">
                <td><strong>${e.name}</strong></td>
                <td>${e.email}</td>
                ...
                <td><span class="status-badge ${t}">${e.status}</span></td>
                <td>
                    <button class="btn btn-sm btn-secondary" onclick="adminFunctions.viewDetails('${e.id}', 'user')">👁️ Ver</button>
                    <button class="btn btn-sm btn-primary" onclick="adminFunctions.editUser('${e.id}')">✏️ Editar</button>
                    <button class="btn btn-sm btn-danger" onclick="adminFunctions.deleteUser('${e.id}')">🗑️ Excluir</button>
                </td>
            </tr>
        `}loadAuditData(){this.populateTable("audit-table-body",[{id:"audit_001",datetime:"2025-10-03 14:30:15",user:"admin@claunnetworkingworkingworking.com",action:"Aprovação",resource:"Vaga",resourceId:"vaga_123",details:'Vaga "Desenvolvedor React" aprovada'},{id:"audit_002",datetime:"2025-10-03 13:45:22",user:"admin@claunnetworkingworkingworking.com",action:"Criação",resource:"Curso",resourceId:"curso_456",details:'Novo curso "Python Básico" criado'},{id:"audit_003",datetime:"2025-10-03 12:15:08",user:"admin@claunnetworkingworkingworking.com",action:"Exclusão",resource:"Usuário",resourceId:"user_789",details:"Usuário spam removido do sistema"}],this.renderAuditRow.bind(this))}renderAuditRow(e){var t="action-"+e.action.toLowerCase();return`
            <tr data-id="${e.id}">
        ...
        `}loadNotificationsData(){this.populateTable("notifications-table-body",[{id:"notif_001",datetime:"2025-10-03 15:20:30",type:"Email",recipient:"empresa@teste.com",status:"Enviado",message:"Confirmação de pagamento do plano Profissional"},{id:"notif_002",datetime:"2025-10-03 14:15:45",type:"Webhook",recipient:"api.payment.com",status:"Falhou",message:"Erro ao processar webhook de pagamento"},{id:"notif_003",datetime:"2025-10-03 13:30:12",type:"SMS",recipient:"+55 11 99999-1234",status:"Enviado",message:"Código de verificação para login"}],this.renderNotificationRow.bind(this))}renderNotificationRow(e){var t="Enviado"===e.status?"status-approved":"status-rejected",a="type-"+e.type.toLowerCase();return`
            <tr data-id="${e.id}">
                <td>${this.formatDateTime(e.datetime)}</td>
                <td><span class="type-badge ${a}">${e.type}</span></td>
                <td>${e.recipient}</td>
                ...
                <div class="culture-overview">
                    <div class="culture-hero">
                        <div class="hero-content">
                            ...
                            <p class="hero-tagline">${t.tagline||"Transformando o futuro através da tecnologia"}</p>
        ...
        `,t.style.display="flex"}getAspectData(e){var t={ambiente_trabalho:{title:"🏢 Ambiente de Trabalho",icon:"🏢",description:"Nossos espaços são projetados para inspirar criatividade, promover colaboração e garantir o conforto de todos.",highlights:[{icon:"🪑",title:"Mobiliário Ergonômico",description:"Cadeiras e mesas ajustáveis para máximo conforto"},{icon:"🌱",title:"Ambiente Verde",description:"Plantas e luz natural em todos os espaços"},{icon:"🔇",title:"Acústica Otimizada",description:"Espaços silenciosos para concentração"},{icon:"☕",title:"Café Premium",description:"Café especial disponível 24/7"}],metrics:[{value:"95%",label:"Satisfação com o ambiente"},{value:"24°C",label:"Temperatura ideal"},{value:"100%",label:"Iluminação natural"}],testimonials:[{text:"O ambiente aqui é incrível! Me sinto motivada todos os dias.",author:"Ana Silva",role:"Desenvolvedora"}]},beneficios:{title:"🎁 Benefícios",icon:"🎁",description:"Oferecemos um pacote completo de benefícios pensado no bem-estar e qualidade de vida de nossos colaboradores.",highlights:[{icon:"🏥",title:"Plano de Saúde Premium",description:"Cobertura completa para você e sua família"},{icon:"🍽️",title:"Vale Refeição",description:"R$ 35/dia para suas refeições"},{icon:"🏋️",title:"Gympass",description:"Acesso a mais de 50.000 academias"},{icon:"📚",title:"Educação",description:"R$ 3.000/ano para cursos e certificações"}],metrics:[{value:"R$ 8.500",label:"Valor médio em benefícios"},{value:"15+",label:"Tipos de benefícios"},{value:"100%",label:"Cobertura do plano de saúde"}],testimonials:[{text:"Os benefícios aqui são realmente diferenciados. Sinto que a empresa se importa comigo.",author:"Carlos Santos",role:"Tech Lead"}]},diversidade:{title:"🌈 Diversidade & Inclusão",icon:"🌈",description:"Acreditamos que a diversidade é nossa força. Promovemos um ambiente inclusivo onde todos podem ser autênticos.",highlights:[{icon:"👥",title:"Equipe Diversa",description:"50% mulheres em posições de liderança"},{icon:"🏳️‍🌈",title:"LGBTQIA+ Friendly",description:"Políticas inclusivas e grupos de apoio"},{icon:"♿",title:"Acessibilidade",description:"Escritório 100% acessível"},{icon:"🌍",title:"Multiculturalismo",description:"Time de 12 nacionalidades diferentes"}],metrics:[{value:"50%",label:"Mulheres na liderança"},{value:"12",label:"Nacionalidades"},{value:"95%",label:"Índice de inclusão"}],testimonials:[{text:"Aqui posso ser eu mesma sem medo. A diversidade é celebrada todos os dias.",author:"Maria Oliveira",role:"UX Designer"}]},inovacao:{title:"💡 Inovação",icon:"💡",description:"Incentivamos a experimentação, criatividade e o pensamento fora da caixa em todos os projetos.",highlights:[{icon:"🚀",title:"Innovation Days",description:"20% do tempo para projetos pessoais"},{icon:"🧪",title:"Lab de Experimentação",description:"Espaço dedicado para testar novas ideias"},{icon:"💰",title:"Fundo de Inovação",description:"R$ 50.000 anuais para projetos inovadores"},{icon:"🏆",title:"Hackathons",description:"Eventos mensais de inovação"}],metrics:[{value:"25",label:"Projetos inovadores/ano"},{value:"20%",label:"Tempo para inovação"},{value:"8",label:"Patentes registradas"}],testimonials:[{text:"Minhas ideias são sempre ouvidas e apoiadas. É incrível ver projetos saindo do papel.",author:"João Pedro",role:"Desenvolvedor Senior"}]},crescimento:{title:"📈 Crescimento",icon:"📈",description:"Investimos no desenvolvimento profissional e pessoal de cada colaborador através de programas estruturados.",highlights:[{icon:"🎯",title:"Plano de Carreira",description:"Trilhas claras de desenvolvimento"},{icon:"👨‍🏫",title:"Mentoria",description:"Programa de mentoria com líderes"},{icon:"📜",title:"Certificações",description:"Apoio financeiro para certificações"},{icon:"🌟",title:"Promoções Internas",description:"80% das vagas preenchidas internamente"}],metrics:[{value:"80%",label:"Promoções internas"},{value:"40h",label:"Treinamento/mês"},{value:"95%",label:"Satisfação com crescimento"}],testimonials:[{text:"Em 2 anos fui promovida duas vezes. O crescimento aqui é real e baseado em mérito.",author:"Fernanda Costa",role:"Product Manager"}]},equilibrio:{title:"⚖️ Work-Life Balance",icon:"⚖️",description:"Promovemos o equilíbrio saudável entre vida pessoal e profissional através de políticas flexíveis.",highlights:[{icon:"🏠",title:"Home Office",description:"Flexibilidade total para trabalho remoto"},{icon:"⏰",title:"Horário Flexível",description:"Escolha seu horário de trabalho"},{icon:"🏖️",title:"Férias Ilimitadas",description:"Tire férias quando precisar"},{icon:"🧘",title:"Bem-estar Mental",description:"Apoio psicológico e meditação"}],metrics:[{value:"4.9/5",label:"Satisfação com equilíbrio"},{value:"30 dias",label:"Férias médias/ano"},{value:"2%",label:"Taxa de burnout"}],testimonials:[{text:"Consigo conciliar perfeitamente minha vida pessoal com o trabalho. A flexibilidade é real.",author:"Roberto Lima",role:"DevOps Engineer"}]}};return t[e]||t.ambiente_trabalho}startVirtualTour(){document.getElementById("virtual-tour-modal").style.display="flex",this.visitSpot("reception")}visitSpot(e){let t={reception:{name:"Recepção",description:"Nossa recepção acolhedora com design moderno",icon:"🏢"},workspace:{name:"Área de Trabalho",description:"Espaços colaborativos e individuais",icon:"💻"},recreation:{name:"Área de Lazer",description:"Espaço para relaxar e socializar",icon:"🎮"},meeting:{name:"Salas de Reunião",description:"Ambientes para colaboração criativa",icon:"🤝"}}[e];t&&(document.getElementById("current-spot").textContent=t.name,document.getElementById("tour-360-view").innerHTML=`
            <div class="tour-placeholder">
...
`;if(!document.getElementById("compatibility-styles")){let style=document.createElement("style");style.id="compatibility-styles",style.textContent=compatibilityCSS,document.head.appendChild(style)}window.compatibilitySystem=new CompatibilitySystem;class CoursePricingLogic{constructor(){this.init()}init(){this.checkUserTypeAndAdjustPricing(),this.addPriceTypeListeners()}checkUserTypeAndAdjustPricing(){var e=this.getCurrentUserType(),t=document.querySelector('input[value="paid"]'),a=t?t.closest(".radio-option"):null;if(a)switch(e){case"candidate":this.restrictToPaidCourses(a);break;case"company":case"admin":this.allowPaidCourses(a);break;default:this.restrictToPaidCourses(a)}}getCurrentUserType(){return window.authSystem&&window.authSystem.isLoggedIn()?window.authSystem.getCurrentUser().type:localStorage.getItem("claunnetworkingworkingworking_user_type")||"guest"}restrictToPaidCourses(e){e.style.opacity="0.5",e.style.pointerEvents="none";var t=e.querySelector('input[type="radio"]'),e=e.querySelector("label"),e=(t&&(t.disabled=!0),e&&(e.innerHTML='💳 Pago <small style="color: #ef4444;">(Indisponível)</small>'),this.addRestrictedPricingWarning(),document.querySelector('input[value="free"]'));e&&t&&t.checked&&(e.checked=!0,e.closest(".radio-option").click())}allowPaidCourses(e){e.style.opacity="1",e.style.pointerEvents="auto";var t=e.querySelector('input[type="radio"]'),e=e.querySelector("label");t&&(t.disabled=!1),e&&(e.innerHTML="💳 Pago"),this.removeRestrictedPricingWarning()}addRestrictedPricingWarning(){var e,t,a;document.querySelector(".pricing-restriction-warning")||((e=document.createElement("div")).className="alert alert-warning pricing-restriction-warning",e.innerHTML=`
        ...
        `,a=(t=document.querySelector(".form-section h3").parentElement).querySelector(".form-group"),t.insertBefore(e,a))}removeRestrictedPricingWarning(){var e=document.querySelector(".pricing-restriction-warning");e&&e.remove()}addPriceTypeListeners(){document.querySelectorAll(".radio-option").forEach(a=>{a.addEventListener("click",e=>{var t=a.querySelector('input[type="radio"]');t&&t.disabled&&(e.preventDefault(),e.stopPropagation(),this.showPaidCourseRestrictionAlert())})})}showPaidCourseRestrictionAlert(){var e=this.getCurrentUserType();let t="";t="candidate"===e?"Como candidato, você pode cadastrar apenas cursos gratuitos. Para publicar cursos pagos, é necessário ter uma conta empresarial com plano ativo.":"Para publicar cursos pagos, é necessário fazer login como empresa com plano ativo ou como administrador.",alert(t)}hasActivePlan(){var e;return window.authSystem&&window.authSystem.isLoggedIn()?(e=window.authSystem.getCurrentUser()).profile&&!0===e.profile.activePlan:"true"===localStorage.getItem("claunnetworkingworkingworking_active_plan")}checkCompanyPlanStatus(){var e,t,a;"company"!==this.getCurrentUserType()||this.hasActivePlan()||(this.restrictToPaidCourses(document.querySelector('input[value="paid"]').closest(".radio-option")),(e=document.createElement("div")).className="alert alert-warning",e.innerHTML=`
    ...
    `,t}function showJobDetails(e){console.log("Mostrar detalhes da vaga:",e)}function applyToJob(e){APIUtils.isLoggedIn()?console.log("Candidatar-se à vaga:",e):APIUtils.showError("Você precisa estar logado para se candidatar.")}function showCourseDetails(e){console.log("Mostrar detalhes do curso:",e)}function enrollInCourse(e){APIUtils.isLoggedIn()?console.log("Inscrever-se no curso:",e):APIUtils.showError("Você precisa estar logado para se inscrever.")}function showRegistrationConfirmModal(e,t){t=`
            ...
            <div style="background: #f8fafc; padding: 1.5rem; border-radius: 12px; margin-bottom: 2rem;">
                ...
                <div style="display: grid; gap: 0.75rem;">
                    <div style="display: flex; justify-content: space-between;">
                        ...
                        <strong style="color: #374151;">${{candidato:"Candidato",empresa:"Empresa",instituicao:"Instituição de Ensino"}[e]}</strong>
                    </div>
                    <div style="display: flex; justify-content: space-between;">
                        <span style="color: #6b7280;">Email:</span>
                        <strong style="color: #374151;">${t.email||"Não informado"}</strong>
                    </div>
                    <div style="display: flex; justify-content: space-between;">
    ...
    `;createModal("Cadastro Concluído",t,{maxWidth:"600px"})}function showValidationModal(e){e=e.map(e=>`<li style="margin-bottom: 0.5rem;">${e}</li>`).join("");createModal("Erro de Validação",`
        <div style="padding: 1rem 0;">
            <div style="text-align: center; margin-bottom: 2rem;">
    ...
    `,{maxWidth:"500px"})}function showTermsModal(e,t){createModal("Termos e Condições",`
        <div style="padding: 1rem 0;">
    ...
    `,{maxWidth:"700px"})}function acceptTerms(onAcceptFunction){let checkbox=document.getElementById("acceptTerms");checkbox.checked?(closeAllModals(),onAcceptFunction&&eval(onAcceptFunction+"()")):showErrorModal("Erro","Você deve aceitar os termos para continuar.")}function showFileUploadModal(e,t,a){t=`
        <div style="padding: 1rem 0;">
            <div style="text-align: center; margin-bottom: 2rem;">
    ...
    `;createModal("Editar Perfil",e,{maxWidth:"600px"})}function saveProfileChanges(onSaveFunction){let form=document.getElementById("editProfileForm"),formData=new FormData(form),data=Object.fromEntries(formData.entries());showLoadingModal("Salvando alterações..."),setTimeout(()=>{hideLoadingModal(),showSuccessModal("Sucesso","Perfil atualizado com sucesso!"),onSaveFunction&&eval(onSaveFunction+"(data)")},1500)}document.addEventListener("DOMContentLoaded",function(){new CoursePricingLogic}),window.CoursePricingLogic=CoursePricingLogic,document.addEventListener("DOMContentLoaded",function(){let e=document.getElementById("loginForm"),t=(e&&e.addEventListener("submit",async function(t){t.preventDefault();t=new FormData(e),t={email:t.get("email"),password:t.get("password")};try{let e=await api.login(t);APIUtils.setCurrentUser(e.user),APIUtils.showSuccess("Login realizado com sucesso!"),setTimeout(()=>{switch(e.user.type){case"candidato":window.location.href="/candidato_painel.html";break;case"empresa":window.location.href="/company_dashboard.html";break;case"instituicao":window.location.href="/institution_dashboard.html";break;case"admin":window.location.href="/admin_complete.html";break;default:window.location.href="/index.html"}},1500)}catch(e){APIUtils.showError(e.message)}}),document.getElementById("registerForm")),a=(t&&t.addEventListener("submit",async function(e){e.preventDefault();e=new FormData(t),e={name:e.get("name"),email:e.get("email"),password:e.get("password"),user_type:e.get("user_type"),phone:e.get("phone")};try{await api.register(e);APIUtils.showSuccess("Cadastro realizado com sucesso!"),t.reset(),setTimeout(()=>{window.location.href="/index.html"},2e3)}catch(e){APIUtils.showError(e.message)}}),document.getElementById("jobForm")),i=(a&&a.addEventListener("submit",async function(e){e.preventDefault();e=new FormData(a),e={title:e.get("title"),description:e.get("description"),requirements:e.get("requirements"),benefits:e.get("benefits"),salary_range:e.get("salary_range"),location:e.get("location"),work_modality:e.get("work_modality"),job_type:e.get("job_type"),area:e.get("area"),level:e.get("level")};try{await api.createJob(e);APIUtils.showSuccess("Vaga criada com sucesso!"),a.reset()}catch(e){APIUtils.showError(e.message)}}),document.getElementById("applicationForm")),o=(i&&i.addEventListener("submit",async function(e){e.preventDefault();e=new FormData(i),e={job_id:e.get("job_id"),message:e.get("message")};try{await api.applyToJob(e);APIUtils.showSuccess("Candidatura enviada com sucesso!");var t=document.querySelector(".modal.show");t&&bootstrap.Modal.getInstance(t).hide()}catch(e){APIUtils.showError(e.message)}}),document.getElementById("courseForm"));o&&o.addEventListener("submit",async function(e){e.preventDefault();e=new FormData(o),e={title:e.get("title"),description:e.get("description"),category:e.get("category"),level:e.get("level"),modality:e.get("modality"),duration:e.get("duration"),price:parseFloat(e.get("price"))||0,is_free:"on"===e.get("is_free")};try{await api.createCourse(e);APIUtils.showSuccess("Curso criado com sucesso!"),o.reset()}catch(e){APIUtils.showError(e.message)}});var s=document.getElementById("logoutBtn");s&&s.addEventListener("click",async function(e){e.preventDefault();try{await api.logout(),APIUtils.clearCurrentUser(),APIUtils.showSuccess("Logout realizado com sucesso!"),setTimeout(()=>{window.location.href="/index.html"},1e3)}catch(e){APIUtils.showError(e.message)}})}),document.addEventListener("DOMContentLoaded",function(){var e=window.location.pathname;(e.includes("vagas.html")||e.includes("buscar_"))&&loadJobs(),(e.includes("cursos")||e.includes("buscar_"))&&loadCourses()}),document.addEventListener("DOMContentLoaded",function(){console.log("Form modals carregado com sucesso!")});class IdentityVerificationSystem{constructor(){this.verificationLevels={basic:{name:"Básico",color:"#6b7280",icon:"📧"},email:{name:"Email Verificado",color:"#3b82f6",icon:"✉️"},phone:{name:"Telefone Verificado",color:"#8b5cf6",icon:"📱"},document:{name:"Documento Verificado",color:"#10b981",icon:"🆔"},professional:{name:"Profissional Verificado",color:"#f59e0b",icon:"💼"},premium:{name:"Verificação Premium",color:"#ef4444",icon:"⭐"}},this.verificationSteps=[{id:"email",name:"Verificar Email",required:!0},{id:"phone",name:"Verificar Telefone",required:!1},{id:"document",name:"Enviar Documento",required:!1},{id:"professional",name:"Verificação Profissional",required:!1},{id:"social",name:"Redes Sociais",required:!1}]}createVerificationModal(){var e=`
                        ...
                        <div class="verification-steps">
                            ${this.verificationSteps.map(e=>`
                                <div class="verification-step" data-step="${e.id}">
                                    <div class="step-icon">
                                        <span class="step-status" id="status-${e.id}">⏳</span>
                                    </div>
                                    <div class="step-content">
                                        <h4>${e.name}</h4>
                                        <p class="step-description" id="desc-${e.id}">Clique para iniciar</p>
                                        ${e.required?'<span class="required-badge">Obrigatório</span>':""}
                                    </div>
        ...
        `,t=document.getElementById("verification-modal");t&&t.remove(),document.body.insertAdjacentHTML("beforeend",e)}startVerification(e){switch(e){case"email":this.verifyEmail();break;case"phone":this.verifyPhone();break;case"document":this.verifyDocument();break;case"professional":this.verifyProfessional();break;case"social":this.verifySocial()}}verifyEmail(){var e=prompt("Digite seu email para verificação:");e&&this.isValidEmail(e)?(this.updateStepStatus("email","loading","Enviando código de verificação..."),setTimeout(()=>{var e=prompt("Digite o código de 6 dígitos enviado para seu email:");e&&6===e.length?(this.updateStepStatus("email","success","Email verificado com sucesso!"),this.saveVerificationStatus("email",!0)):this.updateStepStatus("email","error","Código inválido. Tente novamente.")},2e3)):alert("Email inválido!")}verifyPhone(){var e=prompt("Digite seu telefone (com DDD):");e&&this.isValidPhone(e)?(this.updateStepStatus("phone","loading","Enviando SMS..."),setTimeout(()=>{var e=prompt("Digite o código de 4 dígitos enviado por SMS:");e&&4===e.length?(this.updateStepStatus("phone","success","Telefone verificado com sucesso!"),this.saveVerificationStatus("phone",!0)):this.updateStepStatus("phone","error","Código inválido. Tente novamente.")},2e3)):alert("Telefone inválido!")}verifyDocument(){var e=document.createElement("input");e.type="file",e.accept="image/*,.pdf",e.onchange=e=>{e.target.files[0]&&(this.updateStepStatus("document","loading","Analisando documento..."),setTimeout(()=>{this.updateStepStatus("document","success","Documento verificado com sucesso!"),this.saveVerificationStatus("document",!0)},3e3))},e.click()}verifyProfessional(){var e=prompt("Digite o link do seu perfil LinkedIn:");e&&e.includes("linkedin.com")?(this.updateStepStatus("professional","loading","Verificando perfil profissional..."),setTimeout(()=>{this.updateStepStatus("professional","success","Perfil profissional verificado!"),this.saveVerificationStatus("professional",!0)},2500)):alert("Link do LinkedIn inválido!")}verifySocial(){var e=prompt("Digite o link de uma rede social (Instagram, Twitter, etc.):");e&&(e.includes("instagram.com")||e.includes("twitter.com"))?(this.updateStepStatus("social","loading","Verificando rede social..."),setTimeout(()=>{this.updateStepStatus("social","success","Rede social verificada!"),this.saveVerificationStatus("social",!0)},2e3)):alert("Link de rede social inválido!")}updateStepStatus(e,t,a){var i=document.getElementById("status-"+e),e=document.getElementById("desc-"+e);if(i&&e){switch(t){case"loading":i.textContent="⏳",i.className="step-status loading";break;case"success":i.textContent="✅",i.className="step-status success";break;case"error":i.textContent="❌",i.className="step-status error"}e.textContent=a}}saveVerificationStatus(e,t){var a=JSON.parse(localStorage.getItem("userVerifications")||"{}");a[e]={verified:t,date:(new Date).toISOString()},localStorage.setItem("userVerifications",JSON.stringify(a)),this.updateProfileBadge()}getUserVerificationLevel(){var e=JSON.parse(localStorage.getItem("userVerifications")||"{}");return 4<=Object.values(e).filter(e=>e.verified).length?"premium":e.professional?.verified?"professional":e.document?.verified?"document":e.phone?.verified?"phone":e.email?.verified?"email":"basic"}createVerificationBadge(e=null){var e=e||this.getUserVerificationLevel(),t=this.verificationLevels[e];return`
...
`;if(!document.getElementById("verification-styles")){let style=document.createElement("style");style.id="verification-styles",style.textContent=verificationCSS,document.head.appendChild(style)}window.verificationSystem=new IdentityVerificationSystem;class InternalChatSystem{constructor(){this.conversations=JSON.parse(localStorage.getItem("chatConversations")||"{}"),this.currentConversation=null,this.currentUser=this.getCurrentUser(),this.messageTemplates=this.initializeTemplates(),this.isOpen=!1}getCurrentUser(){return JSON.parse(localStorage.getItem("currentUser")||'{"id": "user1", "name": "Usuário Atual", "type": "candidate"}')}initializeTemplates(){return{candidate_to_company:["Olá! Tenho interesse na vaga de {position}. Gostaria de saber mais detalhes sobre a oportunidade.","Bom dia! Vi que vocês estão contratando para {position}. Meu perfil se encaixa perfeitamente na vaga.","Olá! Gostaria de me candidatar à vaga de {position}. Quando podemos conversar?"],company_to_candidate:["Olá {name}! Seu perfil chamou nossa atenção. Gostaria de agendar uma conversa?","Oi {name}! Temos uma oportunidade que pode interessar você. Vamos conversar?","Olá! Gostaríamos de conhecer melhor seu trabalho. Tem disponibilidade para uma call?"],institution_to_candidate:["Olá {name}! Temos cursos que podem acelerar sua carreira. Gostaria de saber mais?","Oi! Nossos cursos de {area} estão com inscrições abertas. Interesse?","Olá! Que tal investir em sua capacitação? Temos ótimas opções para você."]}}createChatInterface(){var e=document.getElementById("chat-system");e&&e.remove(),document.body.insertAdjacentHTML("beforeend",`
        ...
        `).join(""),e.style.display="flex"}useTemplate(e){var t=document.getElementById("message-input");t&&(t.value=e.replace("{name}","Nome").replace("{position}","Posição").replace("{area}","Área"),t.focus()),this.closeModal("templates-modal")}showScheduleModal(){document.getElementById("schedule-modal").style.display="flex";var e=(new Date).toISOString().split("T")[0];document.getElementById("meeting-date").min=e}scheduleMeeting(){var e=document.getElementById("meeting-title").value,t=document.getElementById("meeting-date").value,a=document.getElementById("meeting-time").value,i=document.getElementById("meeting-duration").value,o=document.getElementById("meeting-type").value,s=document.getElementById("meeting-notes").value;e&&t&&a?(e={id:Date.now().toString(),text:"📅 Reunião agendada: "+e,sender:this.currentUser.id,timestamp:(new Date).toISOString(),type:"meeting",meetingData:{title:e,date:t,time:a,duration:i,type:o,notes:s}},this.conversations[this.currentConversation].messages.push(e),this.conversations[this.currentConversation].lastMessage=e,this.saveConversations(),this.loadMessages(this.currentConversation),this.closeModal("schedule-modal"),document.getElementById("meeting-title").value="",document.getElementById("meeting-date").value="",document.getElementById("meeting-time").value="",document.getElementById("meeting-notes").value=""):alert("Preencha todos os campos obrigatórios!")}showConversationsList(){document.getElementById("chat-messages").style.display="none",document.getElementById("conversations-list").style.display="block",this.currentConversation=null}closeModal(e){document.getElementById(e).style.display="none"}updateUnreadBadge(){var e=document.getElementById("chat-badge"),t=Object.values(this.conversations).reduce((e,t)=>e+(t.unreadCount||0),0);0<t?(e.textContent=t,e.style.display="block"):e.style.display="none"}saveConversations(){localStorage.setItem("chatConversations",JSON.stringify(this.conversations))}formatTime(e){var t;return e?(e=new Date(e),t=new Date,0===(t=Math.floor((t-e)/864e5))?e.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}):1===t?"Ontem":t<7?t+" dias":e.toLocaleDateString("pt-BR")):""}getTypeLabel(e){return{company:"Empresa",candidate:"Candidato",institution:"Instituição"}[e]||e}init(){this.createChatInterface(),0===Object.keys(this.conversations).length&&this.addSampleConversations()}addSampleConversations(){var e={conv_1:{id:"conv_1",participant:{id:"company1",name:"TechCorp Brasil",type:"company"},messages:[{id:"1",text:"Olá! Seu perfil chamou nossa atenção para a vaga de Desenvolvedor Frontend.",sender:"company1",timestamp:new Date(Date.now()-36e5).toISOString(),type:"text"}],lastMessage:{text:"Olá! Seu perfil chamou nossa atenção para a vaga de Desenvolvedor Frontend.",timestamp:new Date(Date.now()-36e5).toISOString()},unreadCount:1,createdAt:new Date(Date.now()-36e5).toISOString()}};this.conversations=e,this.saveConversations()}}let chatCSS=`
.chat-system {
    position: fixed;
    bottom: 20px;
    right: 20px;
...
`;if(!document.getElementById("chat-styles")){let style=document.createElement("style");style.id="chat-styles",style.textContent=chatCSS,document.head.appendChild(style)}window.chatSystem=new InternalChatSystem;class JobEngagement{constructor(){this.engagementData=this.loadEngagementData(),this.init()}init(){this.addEngagementListeners()}loadEngagementData(){var e=localStorage.getItem("claunnetworkingworkingworking_job_engagement");return e?JSON.parse(e):{}}saveEngagementData(){localStorage.setItem("claunnetworkingworkingworking_job_engagement",JSON.stringify(this.engagementData))}addEngagementListeners(){document.addEventListener("click",e=>{var t,a;e.target.closest(".engagement-item")&&(t=(e=e.target.closest(".engagement-item")).closest(".job-card"),t=this.getJobIdFromCard(t),a=this.getEngagementType(e),t)&&a&&this.handleEngagement(t,a,e)})}getJobIdFromCard(e){var e=e.querySelector(".btn-details");return e&&e.onclick&&(e=e.getAttribute("onclick").match(/viewJobDetails\((\d+)\)/))?parseInt(e[1]):null}getEngagementType(e){switch(e.querySelector("span:first-child").textContent){case"❤️":return"likes";case"👁️":return"views";case"🔗":return"shares";default:return null}}handleEngagement(e,t,a){switch(this.engagementData[e]||(this.engagementData[e]={likes:!1,views:!1,shares:!1}),t){case"likes":this.toggleLike(e,a);break;case"views":this.incrementViews(e,a);break;case"shares":this.incrementShares(e,a)}this.saveEngagementData()}toggleLike(e,t){var a=this.engagementData[e].likes,i=(this.engagementData[e].likes=!a,t.querySelector("span:first-child"));t.querySelector("span:last-child");a?(i.textContent="❤️",t.style.color=""):(i.textContent="💖",t.style.color="#ef4444",this.animateEngagement(t,"+1")),this.updateGlobalCounter(e,"likes",a?-1:1)}incrementViews(e,t){this.animateEngagement(t,"+1 view"),this.updateGlobalCounter(e,"views",1)}incrementShares(e,t){this.animateEngagement(t,"+1 share"),this.updateGlobalCounter(e,"shares",1)}updateGlobalCounter(t,e,a){var i=(window.jobsData||[]).find(e=>e.id===t);i&&i.engagement&&(i.engagement[e]+=a,this.updateDisplayCounter(t,e,i.engagement[e]))}updateDisplayCounter(t,a,i){document.querySelectorAll(".job-card").forEach(e=>{this.getJobIdFromCard(e)===t&&e.querySelectorAll(".engagement-item").forEach(e=>{this.getEngagementType(e)===a&&(e.querySelector("span:last-child").textContent="views"===a?i+" visualizações":i.toString())})})}animateEngagement(e,t){let a=document.createElement("div");a.textContent=t,a.style.cssText=`
            position: absolute;
            background: #10B981;
            color: white;
...
`;if(!document.getElementById("mobile-optimization-styles")){let style=document.createElement("style");style.id="mobile-optimization-styles",style.textContent=mobileOptimizationCSS,document.head.appendChild(style)}document.addEventListener("DOMContentLoaded",()=>{window.mobileOptimization=new MobileOptimizationSystem}),window.MobileOptimizationSystem=MobileOptimizationSystem;class PortfolioSystem{constructor(){this.portfolioData=JSON.parse(localStorage.getItem("userPortfolio")||"{}"),this.supportedTypes=["image","video","document","link","code"],this.maxFileSize=10485760,this.githubIntegration=!1,this.behanceIntegration=!1}createPortfolioInterface(e){e=document.getElementById(e);e&&(e.innerHTML=`
        ...
        `,this.attachEventListeners(),this.loadPortfolio(),this.updateStats())}attachEventListeners(){var e=document.getElementById("project-form"),e=(e&&e.addEventListener("submit",e=>{e.preventDefault(),this.addProject()}),document.getElementById("project-files"));e&&e.addEventListener("change",e=>{this.handleFileSelection(e.target.files)});let t=document.querySelectorAll(".filter-btn");t.forEach(e=>{e.addEventListener("click",e=>{this.filterProjects(e.target.dataset.filter),t.forEach(e=>e.classList.remove("active")),e.target.classList.add("active")})})}handleFileSelection(e){let i=document.getElementById("selected-files");i&&(i.innerHTML="",Array.from(e).forEach((e,t)=>{var a;e.size>this.maxFileSize?alert(`Arquivo ${e.name} é muito grande. Máximo 10MB.`):((a=document.createElement("div")).className="file-item",a.innerHTML=`
            ...
            <div class="project-card" data-category="${e.category}" onclick="portfolioSystem.showProjectDetail('${e.id}')">
                <div class="project-image">
                    ...
                    ${e.featured?'<div class="featured-badge">⭐ Destaque</div>':""}
                </div>
                <div class="project-content">
                    ...
                    <div class="project-stats">
                        ...
                        <span class="stat">❤️ ${e.likes}</span>
                        ${e.link?'<span class="stat">🔗 Link</span>':""}
                        ${e.github?'<span class="stat">🐙 GitHub</span>':""}
                    </div>
                </div>
            </div>
                ...
                <div class="project-info">
                    ...
                    <div class="info-section">
                        ...
                        <div class="project-links">
                            ${o.link?`<a href="${o.link}" target="_blank" class="project-link">🌐 Ver Projeto</a>`:""}
                            ${o.github?`<a href="${o.github}" target="_blank" class="project-link">🐙 GitHub</a>`:""}
                        </div>
                    </div>
        ...
        `,e.style.display="flex")}filterProjects(t){document.querySelectorAll(".project-card").forEach(e=>{"all"===t||e.dataset.category===t?e.style.display="block":e.style.display="none"})}connectGitHub(){var e=prompt("Digite seu username do GitHub:");e&&(this.githubIntegration=!0,alert("GitHub conectado! Username: "+e),this.updateIntegrationStatus("github",!0))}connectBehance(){var e=prompt("Digite seu username do Behance:");e&&(this.behanceIntegration=!0,alert("Behance conectado! Username: "+e),this.updateIntegrationStatus("behance",!0))}connectLinkedIn(){alert("Redirecionando para autenticação do LinkedIn..."),setTimeout(()=>{alert("LinkedIn conectado com sucesso!"),this.updateIntegrationStatus("linkedin",!0)},2e3)}updateIntegrationStatus(e,t){var e=document.getElementById(e+"-integration");e&&(e=e.querySelector(".integration-btn"),t)&&(e.textContent="Conectado ✓",e.style.background="#10b981",e.disabled=!0)}syncGitHub(){let t=document.getElementById("github-username").value;t?(alert("Sincronizando repositórios..."),setTimeout(()=>{var e=[{title:"E-commerce React App",category:"web",description:"Aplicação de e-commerce completa com React e Node.js",technologies:["React","Node.js","MongoDB","Stripe"],github:`https://github.com/${t}/ecommerce-app`},{title:"Task Manager API",category:"web",description:"API RESTful para gerenciamento de tarefas",technologies:["Node.js","Express","PostgreSQL"],github:`https://github.com/${t}/task-manager-api`}];e.forEach(e=>{e={id:Date.now().toString()+Math.random(),...e,files:[],createdAt:(new Date).toISOString(),views:Math.floor(100*Math.random()),likes:Math.floor(20*Math.random()),featured:!1};this.portfolioData.projects||(this.portfolioData.projects=[]),this.portfolioData.projects.push(e)}),this.savePortfolio(),this.loadPortfolio(),this.updateStats(),this.closeModal("integrations-modal"),alert(e.length+" repositórios importados com sucesso!")},2e3)):alert("Digite seu username do GitHub!")}syncBehance(){let t=document.getElementById("behance-username").value;t?(alert("Importando projetos do Behance..."),setTimeout(()=>{var e=[{title:"Brand Identity Design",category:"design",description:"Identidade visual completa para startup de tecnologia",technologies:["Photoshop","Illustrator","Figma"],link:`https://behance.net/${t}/brand-identity`}];e.forEach(e=>{e={id:Date.now().toString()+Math.random(),...e,files:[],createdAt:(new Date).toISOString(),views:Math.floor(200*Math.random()),likes:Math.floor(50*Math.random()),featured:!0};this.portfolioData.projects||(this.portfolioData.projects=[]),this.portfolioData.projects.push(e)}),this.savePortfolio(),this.loadPortfolio(),this.updateStats(),this.closeModal("integrations-modal"),alert(e.length+" projetos importados do Behance!")},2e3)):alert("Digite seu username do Behance!")}updateStats(){var e=this.portfolioData.projects||[],t=e.reduce((e,t)=>e+(t.views||0),0),a=e.reduce((e,t)=>e+(t.likes||0),0),i=this.calculatePortfolioScore(e);document.getElementById("projects-count").textContent=e.length,document.getElementById("views-count").textContent=t,document.getElementById("likes-count").textContent=a,document.getElementById("portfolio-score").textContent=i}calculatePortfolioScore(e){var t;return 0===e.length?0:(t=0,t=(t=(t=(t=(t+=10*e.length)+.1*e.reduce((e,t)=>e+(t.views||0),0))+2*e.reduce((e,t)=>e+(t.likes||0),0))+5*e.filter(e=>e.github).length)+5*e.filter(e=>e.link).length,Math.round(t))}getCategoryIcon(e){return{web:"🌐",mobile:"📱",design:"🎨",data:"📊",other:"💼"}[e]||"📁"}getFileIcon(e){return e.startsWith("image/")?"🖼️":e.startsWith("video/")?"🎥":e.includes("pdf")?"📄":"📁"}formatFileSize(e){var t;return 0===e?"0 Bytes":(t=Math.floor(Math.log(e)/Math.log(1024)),parseFloat((e/Math.pow(1024,t)).toFixed(2))+" "+["Bytes","KB","MB","GB"][t])}formatDate(e){return new Date(e).toLocaleDateString("pt-BR")}showAddProjectModal(){document.getElementById("add-project-modal").style.display="flex"}showIntegrationsModal(){document.getElementById("integrations-modal").style.display="flex"}closeModal(e){document.getElementById(e).style.display="none"}likeProject(){alert("Projeto curtido!")}shareProject(){navigator.share?navigator.share({title:"Confira este projeto",url:window.location.href}):(navigator.clipboard.writeText(window.location.href),alert("Link copiado para a área de transferência!"))}toggleViewMode(){var e=document.getElementById("portfolio-grid");e.classList.toggle("grid-view"),e.classList.toggle("list-view")}savePortfolio(){localStorage.setItem("userPortfolio",JSON.stringify(this.portfolioData))}init(){}}let portfolioCSS=`
.portfolio-system {
    background: #f8fafc;
    border-radius: 16px;
    padding: 24px;
...
`;if(!document.getElementById("portfolio-styles")){let style=document.createElement("style");style.id="portfolio-styles",style.textContent=portfolioCSS,document.head.appendChild(style)}function showResponsibilityAlert(){let t=document.createElement("div");t.id="responsibilityModal",t.style.cssText=`
    ...
    `,document.head.appendChild(e),document.body.appendChild(t),setTimeout(()=>{var e=t.querySelector('button[onclick="acceptResponsibility()"]');e&&e.focus()},100)}function acceptResponsibility(){document.getElementById("dontShowAgain").checked&&localStorage.setItem("claunnetworkingworkingworking_responsibility_accepted","true"),closeResponsibilityAlert(),showSuccessMessage("Termos aceitos com sucesso! Você pode prosseguir com a publicação.")}function closeResponsibilityAlert(){let e=document.getElementById("responsibilityModal");e&&(e.style.animation="fadeOut 0.3s ease",setTimeout(()=>{document.body.removeChild(e)},300))}function checkResponsibilityAcceptance(){return"true"===localStorage.getItem("claunnetworkingworkingworking_responsibility_accepted")}function showSuccessMessage(e){let t=document.createElement("div");t.style.cssText=`
        position: fixed;
        top: 20px;
        right: 20px;
        background: #10B981;
    ...
    `,document.head.appendChild(e),document.body.appendChild(t),setTimeout(()=>{t.style.animation="slideOut 0.3s ease",setTimeout(()=>{document.body.contains(t)&&document.body.removeChild(t)},300)},4e3)}function validateContentResponsibility(e){checkResponsibilityAcceptance()?e&&e():(showResponsibilityAlert(),window.originalAcceptResponsibility=window.acceptResponsibility,window.acceptResponsibility=function(){window.originalAcceptResponsibility(),e&&e(),window.acceptResponsibility=window.originalAcceptResponsibility})}window.portfolioSystem=new PortfolioSystem,window.showResponsibilityAlert=showResponsibilityAlert,window.acceptResponsibility=acceptResponsibility,window.closeResponsibilityAlert=closeResponsibilityAlert,window.validateContentResponsibility=validateContentResponsibility;class SalaryTransparencySystem{constructor(){this.marketData=this.initializeMarketData(),this.benefits=this.initializeBenefits()}initializeMarketData(){return{"desenvolvedor-frontend":{min:4500,max:12e3,median:7500},"desenvolvedor-backend":{min:5e3,max:15e3,median:8500},"desenvolvedor-fullstack":{min:5500,max:16e3,median:9e3},"designer-ux-ui":{min:4e3,max:11e3,median:6800},"analista-dados":{min:5500,max:14e3,median:8200},"gerente-projetos":{min:7e3,max:18e3,median:11e3},"marketing-digital":{min:3500,max:9e3,median:5800},vendas:{min:3e3,max:12e3,median:6500},"recursos-humanos":{min:4e3,max:1e4,median:6200},financeiro:{min:4500,max:13e3,median:7800}}}initializeBenefits(){return{"plano-saude":{value:350,description:"Plano de saúde completo"},"plano-odonto":{value:80,description:"Plano odontológico"},"vale-refeicao":{value:600,description:"Vale refeição mensal"},"vale-alimentacao":{value:400,description:"Vale alimentação mensal"},"vale-transporte":{value:220,description:"Vale transporte mensal"},gympass:{value:120,description:"Acesso a academias"},"home-office":{value:200,description:"Auxílio home office"},plr:{value:1500,description:"Participação nos lucros (anual)"},bonus:{value:2e3,description:"Bônus por performance (anual)"},treinamentos:{value:300,description:"Investimento em capacitação"},convenios:{value:150,description:"Convênios e descontos"},"seguro-vida":{value:100,description:"Seguro de vida"}}}calculateBenefitsValue(e){let a=0,i=0;return e.forEach(e=>{var t=this.benefits[e];t&&("plr"===e||"bonus"===e?i+=t.value:a+=t.value)}),{monthly:a,annual:i,monthlyEquivalent:a+i/12}}createBenefitsCalculator(e,a=[]){var t,e=document.getElementById(e);e&&(t=`
        ...
        `,a.innerHTML=t):a.innerHTML="<p>Dados de mercado não disponíveis para esta posição.</p>")}calculateMarketPosition(e,t){var a=Math.min(100,Math.max(0,(e-t.min)/(t.max-t.min)*100));let i,o;o=80<=a?(i="Acima do Mercado","above-market"):40<=a?(i="Dentro da Média","average-market"):(i="Abaixo do Mercado","below-market");e-=t.median,t=0<=e?"+R$ "+e.toLocaleString("pt-BR"):"-R$ "+Math.abs(e).toLocaleString("pt-BR");return{percentile:a,position:i,class:o,difference:t}}createSalaryTransparencySection(e,t){t=document.getElementById(t);t&&(e=`
...
`;if(!document.getElementById("salary-transparency-styles")){let style=document.createElement("style");style.id="salary-transparency-styles",style.textContent=salaryTransparencyCSS,document.head.appendChild(style)}window.salaryTransparencySystem=new SalaryTransparencySystem;class ServicesManager{constructor(){this.storageKey="claunnetworkingworkingworking_services",this.listeners=[],this.initializeDefaultServices(),this.setupStorageListener()}initializeDefaultServices(){var e;localStorage.getItem(this.storageKey)||(e={candidate:[{id:"curriculo-revisao",title:"Revisão de Currículo",description:"Análise completa e otimização do seu currículo por especialistas em RH.",price:89.9,features:["Análise detalhada do currículo","Sugestões de melhorias","Formatação profissional","Palavras-chave otimizadas"],category:"premium",active:!0,partnerUrl:"https://parceiro.com.br/curriculo-revisao",createdAt:(new Date).toISOString(),updatedAt:(new Date).toISOString()},{id:"mentoria-carreira",title:"Mentoria de Carreira",description:"Sessão individual com mentor especializado para acelerar sua carreira.",price:199.9,features:["Sessão de 1h com mentor","Plano de carreira personalizado","Networking estratégico","Acompanhamento por 30 dias"],category:"premium",active:!0,partnerUrl:"https://parceiro.com.br/mentoria-carreira",createdAt:(new Date).toISOString(),updatedAt:(new Date).toISOString()},{id:"simulacao-entrevista",title:"Simulação de Entrevista",description:"Treinamento prático com feedback detalhado para entrevistas de emprego.",price:149.9,features:["Simulação realística","Feedback detalhado","Dicas de postura e comunicação","Gravação da sessão"],category:"premium",active:!0,partnerUrl:"https://parceiro.com.br/simulacao-entrevista",createdAt:(new Date).toISOString(),updatedAt:(new Date).toISOString()}],company:[{id:"divulgacao-premium",title:"Divulgação Premium de Vagas",description:"Destaque suas vagas no topo da plataforma e alcance mais candidatos qualificados.",price:299.9,features:["Posição de destaque por 30 dias","Alcance 5x maior",'Badge "Vaga Premium"',"Relatório de performance"],category:"premium",active:!0,partnerUrl:"https://parceiro.com.br/divulgacao-premium",createdAt:(new Date).toISOString(),updatedAt:(new Date).toISOString()},{id:"analise-candidatos",title:"Análise de Perfil de Candidatos",description:"Receba análises detalhadas dos perfis dos candidatos com scoring automático.",price:199.9,features:["Scoring automático de candidatos","Análise de compatibilidade","Relatório de soft skills","Recomendações de contratação"],category:"premium",active:!0,partnerUrl:"https://parceiro.com.br/analise-candidatos",createdAt:(new Date).toISOString(),updatedAt:(new Date).toISOString()},{id:"consultoria-rh",title:"Consultoria em Recrutamento",description:"Consultoria especializada para otimizar seus processos de recrutamento e seleção.",price:499.9,features:["Sessão de 2h com especialista","Análise do processo atual","Plano de melhoria personalizado","Suporte por 30 dias"],category:"premium",active:!0,partnerUrl:"https://parceiro.com.br/consultoria-rh",createdAt:(new Date).toISOString(),updatedAt:(new Date).toISOString()},{id:"entrevistas-video",title:"Entrevistas por Vídeo",description:"Plataforma integrada para realizar entrevistas por vídeo com candidatos.",price:149.9,features:["Sala de entrevista virtual","Gravação das entrevistas","Compartilhamento com equipe","Agendamento automático"],category:"premium",active:!0,partnerUrl:"https://parceiro.com.br/entrevistas-video",createdAt:(new Date).toISOString(),updatedAt:(new Date).toISOString()}]},this.saveServices(e))}setupStorageListener(){window.addEventListener("storage",e=>{e.key===this.storageKey&&this.notifyListeners()}),window.addEventListener("services-updated",()=>{this.notifyListeners()})}saveServices(e){localStorage.setItem(this.storageKey,JSON.stringify(e)),this.dispatchUpdateEvent()}loadServices(){var e=localStorage.getItem(this.storageKey);return e?JSON.parse(e):{candidate:[],company:[]}}dispatchUpdateEvent(){window.dispatchEvent(new CustomEvent("services-updated"))}addListener(e){this.listeners.push(e)}removeListener(t){this.listeners=this.listeners.filter(e=>e!==t)}notifyListeners(){this.listeners.forEach(e=>e(this.loadServices()))}getAllServices(){return this.loadServices()}getServicesByType(e){return this.loadServices()[e]||[]}getServiceById(t,e=null){var a=this.loadServices();return e?a[e]?.find(e=>e.id===t):(e=a.candidate?.find(e=>e.id===t),a=a.company?.find(e=>e.id===t),e||a)}addService(e,t){var a=this.loadServices(),t={id:this.generateId(),...t,createdAt:(new Date).toISOString(),updatedAt:(new Date).toISOString(),active:!0};return a[e]||(a[e]=[]),a[e].push(t),this.saveServices(a),t}updateService(t,e,a){var i,o=this.loadServices();return!o[e]||-1===(i=o[e].findIndex(e=>e.id===t))?null:(o[e][i]={...o[e][i],...a,updatedAt:(new Date).toISOString()},this.saveServices(o),o[e][i])}removeService(t,e){var a,i=this.loadServices();return!!i[e]&&-1!==(a=i[e].findIndex(e=>e.id===t))&&(i[e].splice(a,1),this.saveServices(i),!0)}toggleServiceStatus(t,e){var a,i=this.loadServices();return!i[e]||-1===(a=i[e].findIndex(e=>e.id===t))?null:(i[e][a].active=!i[e][a].active,i[e][a].updatedAt=(new Date).toISOString(),this.saveServices(i),i[e][a])}generateId(){return"service_"+Date.now()+"_"+Math.random().toString(36).substr(2,9)}getActiveServices(e){return this.getServicesByType(e).filter(e=>e.active)}searchServices(e,t=null){var a=t?this.getServicesByType(t):this.getAllServices();let i=e.toLowerCase();e=e=>e.filter(e=>e.title.toLowerCase().includes(i)||e.description.toLowerCase().includes(i)||e.features.some(e=>e.toLowerCase().includes(i)));return t?e(a):{candidate:e(a.candidate||[]),company:e(a.company||[])}}exportServices(){return{services:this.loadServices(),exportDate:(new Date).toISOString(),version:"1.0"}}importServices(e){return!!e.services&&(this.saveServices(e.services),!0)}}window.servicesManager=new ServicesManager,"undefined"!=typeof module&&module.exports&&(module.exports=ServicesManager);class SmartPersonalizationSystem{constructor(){this.userProfile=this.loadUserProfile(),this.behaviorData=this.loadBehaviorData(),this.preferences=this.loadPreferences(),this.recommendations=[],this.contentFilters={},this.init()}init(){this.detectUserType(),this.trackUserBehavior(),this.personalizeContent(),this.setupRecommendationEngine(),this.initializeAdaptiveUI(),this.startLearningLoop()}detectUserType(){var e=window.location.pathname,t=(navigator.userAgent,(new Date).getHours());e.includes("candidato")?(this.userProfile.type="candidate",this.userProfile.context="job_search"):e.includes("empresa")||e.includes("company")?(this.userProfile.type="company",this.userProfile.context="recruitment"):e.includes("instituicao")||e.includes("institution")?(this.userProfile.type="institution",this.userProfile.context="education"):e.includes("admin")&&(this.userProfile.type="admin",this.userProfile.context="management"),this.userProfile.device=this.detectDevice(),this.userProfile.timeOfDay=this.getTimeContext(t),this.saveUserProfile()}detectDevice(){var e=window.innerWidth;return e<=768?"mobile":e<=1024?"tablet":"desktop"}getTimeContext(e){return 6<=e&&e<12?"morning":12<=e&&e<18?"afternoon":18<=e&&e<22?"evening":"night"}trackUserBehavior(){document.addEventListener("click",e=>{this.recordInteraction("click",{element:e.target.tagName,className:e.target.className,id:e.target.id,text:e.target.textContent?.substring(0,50),timestamp:Date.now()})}),this.startTime=Date.now(),window.addEventListener("beforeunload",()=>{var e=Date.now()-this.startTime;this.recordInteraction("page_time",{duration:e,page:window.location.pathname,timestamp:Date.now()})});let e;window.addEventListener("scroll",()=>{clearTimeout(e),e=setTimeout(()=>{var e=window.scrollY/(document.body.scrollHeight-window.innerHeight)*100;this.recordInteraction("scroll",{percent:Math.round(e),timestamp:Date.now()})},100)}),document.addEventListener("focus",e=>{"INPUT"!==e.target.tagName&&"TEXTAREA"!==e.target.tagName||this.recordInteraction("focus",{field:e.target.name||e.target.id,type:e.target.type,timestamp:Date.now()})},!0),document.querySelectorAll('input[type="search"], input[name*="search"], input[placeholder*="pesquis"]').forEach(e=>{e.addEventListener("input",e=>{2<e.target.value.length&&this.recordInteraction("search",{query:e.target.value,field:e.target.name||e.target.id,timestamp:Date.now()})})})}recordInteraction(e,t){this.behaviorData.interactions||(this.behaviorData.interactions=[]),this.behaviorData.interactions.push({type:e,data:t,sessionId:this.getSessionId(),userType:this.userProfile.type}),1e3<this.behaviorData.interactions.length&&(this.behaviorData.interactions=this.behaviorData.interactions.slice(-1e3)),this.saveBehaviorData(),this.updatePersonalization()}personalizeContent(){this.personalizeNavigation(),this.personalizeCards(),this.personalizeRecommendations(),this.personalizeFilters(),this.personalizeLayout()}personalizeNavigation(){var e=document.querySelector("nav, .navigation");if(e){this.userProfile.type;let a=this.getFrequentPages();var t=e.querySelectorAll("a"),t=Array.from(t);t.sort((e,t)=>{e=a[e.getAttribute("href")]||0;return(a[t.getAttribute("href")]||0)-e}),t.forEach((e,t)=>{t<3&&5<a[e.getAttribute("href")]&&(e.querySelector(".frequent-indicator")||((t=document.createElement("span")).className="frequent-indicator",t.innerHTML="⭐",t.title="Página frequente",e.appendChild(t)))}),this.addPersonalizedShortcuts(e)}}addPersonalizedShortcuts(e){var t,a=this.generateShortcuts();0<a.length&&((t=document.createElement("div")).className="personalized-shortcuts",t.innerHTML=`
            ...
            `,e.appendChild(t))}generateShortcuts(){var e=this.userProfile.type,t=this.getRecentSearches(),a=[];return"candidate"===e?(t.some(e=>e.includes("desenvolvedor"))&&a.push({url:"/vagas.html?area=tecnologia",icon:"💻",text:"Vagas Tech",description:"Vagas em tecnologia baseadas no seu interesse"}),a.push({url:"/candidato_painel.html#profile",icon:"👤",text:"Meu Perfil",description:"Acesso rápido ao seu perfil"})):"company"===e&&(a.push({url:"/buscar_candidatos.html",icon:"🔍",text:"Buscar Talentos",description:"Encontre candidatos ideais"}),a.push({url:"/company_dashboard.html#analytics",icon:"📊",text:"Métricas",description:"Acompanhe suas métricas de recrutamento"})),a.slice(0,4)}personalizeCards(){document.querySelectorAll('.card, [class*="card"]').forEach(e=>{this.enhanceCard(e)})}enhanceCard(e){var t=this.extractCardData(e),a=this.calculateRelevance(t);.7<a?(e.classList.add("high-relevance"),this.addRelevanceIndicator(e,"high")):.4<a&&(e.classList.add("medium-relevance"),this.addRelevanceIndicator(e,"medium")),this.addContextualInfo(e,t),this.personalizeCardActions(e,t)}extractCardData(e){return{title:e.querySelector("h1, h2, h3, h4, h5, h6")?.textContent||"",content:e.textContent||"",links:Array.from(e.querySelectorAll("a")).map(e=>e.href),type:this.inferCardType(e)}}inferCardType(e){e=e.textContent.toLowerCase();return e.includes("vaga")||e.includes("emprego")?"job":e.includes("empresa")||e.includes("company")?"company":e.includes("curso")||e.includes("educação")?"course":e.includes("candidato")||e.includes("perfil")?"candidate":"general"}calculateRelevance(t){let a=0;var e=this.getUserInterests(),i=this.getRecentSearches(),e=(e.forEach(e=>{t.content.toLowerCase().includes(e.toLowerCase())&&(a+=e.weight||.3)}),i.forEach(e=>{t.content.toLowerCase().includes(e.toLowerCase())&&(a+=.2)}),this.userProfile.type);return"candidate"===e&&"job"===t.type&&(a+=.3),"company"===e&&"candidate"===t.type&&(a+=.3),"institution"===e&&"course"===t.type&&(a+=.3),Math.min(a,1)}addRelevanceIndicator(e,t){var a;e.querySelector(".relevance-indicator")||(t={high:{icon:"🎯",text:"Alta compatibilidade",color:"#10b981"},medium:{icon:"✨",text:"Boa compatibilidade",color:"#f59e0b"}}[t],(a=document.createElement("div")).className="relevance-indicator",a.innerHTML=`
        ...
        `,e.style.position="relative",e.appendChild(a))}addContextualInfo(e,t){var a,t=this.generateContextInfo(t);t&&((a=document.createElement("div")).className="contextual-info",a.innerHTML=t,a.style.cssText=`
            ...
            `,e.appendChild(a))}generateContextInfo(e){var t=this.userProfile.type,a=this.userProfile.timeOfDay;if("candidate"===t&&"job"===e.type){var i=this.getMatchReasons(e);if(0<i.length)return"💡 Compatível porque: "+i.join(", ")}return"morning"===a&&"course"===e.type?"🌅 Ótimo momento para aprender algo novo!":"company"===t&&"candidate"===e.type?"🎯 Candidato com perfil alinhado às suas buscas recentes":null}getMatchReasons(t){let a=[];var e=this.getUserSkills(),i=this.getUserInterests();return e.forEach(e=>{t.content.toLowerCase().includes(e.toLowerCase())&&a.push("tem experiência em "+e)}),i.forEach(e=>{t.content.toLowerCase().includes(e.toLowerCase())&&a.push("interesse em "+e)}),a.slice(0,2)}personalizeCardActions(e,t){let a=e.querySelector(".card-actions, .actions");a&&this.generatePersonalizedActions(t).forEach(e=>{var t=document.createElement("button");t.className="personalized-action",t.innerHTML=e.icon+" "+e.text,t.onclick=e.handler,t.style.cssText=`
                background: #8b5cf6;
                color: white;
                border: none;
                padding: 6px 12px;
                ...
                ${this.recommendations.slice(0,5).map(e=>`
                    <div class="recommendation-item" data-type="${e.type}">
                        <div class="recommendation-content">
                            ...
                            <p class="recommendation-reason">💡 ${e.reason}</p>
                        </div>
        ...
        `,this.styleRecommendationsContainer(e),e)}formatRecommendationDetails(e){switch(e.type){case"job":return e.company+" • "+e.location;case"course":return e.provider+" • "+e.duration;case"company":return e.sector+" • "+e.size;case"candidate":return e.skills.join(", ")+" • "+e.experience;case"service":return e.features.join(", ");case"student":return e.course+" • "+e.semester;case"partnership":return e.type+" • "+e.benefits.join(", ");default:return""}}styleRecommendationsContainer(e){e.style.cssText=`
            position: fixed;
            top: 20px;
            right: 20px;
            width: 350px;
